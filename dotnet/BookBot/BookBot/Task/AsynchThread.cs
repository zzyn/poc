...
namespace AsynchThread
{
    ...
    abstract public class Task
    {
        ...
        protected void ActivateStatusChangedEvent(TaskStatus status, object result)
        {
        ...
        protected void ActivateProgressChangedEvent(int progress, object result)
        {
        ...
        /// <param name="progress">任务进度(0-100)</param>
        /// <param name="result">任务工作中间结果对象</param>
        protected void ActivateThrowErrorEvent(int progress, object result)
        {
        ...
        /// <param name="progress">任务进度(0-100)</param>
        /// <param name="result">任务工作中间结果对象</param>
        protected void ActivateCancelEvent(int progress, object result)
        {
            if (TaskCancel != null)
            {
                TaskEventArgs args = new TaskEventArgs(progress, result);
        ...
        /// <param name="progress">任务进度(0-100)</param>
        /// <param name="result">任务工作中间结果对象</param>
        protected void ActivateAbortEvent(int progress, object result)
        {
            if (TaskAbort != null)
            {
                TaskEventArgs args = new TaskEventArgs(progress, result);
        ...
        protected void AsyncInvoke(TaskEventHandler eventhandler, TaskEventArgs args)
        {
            ...
            foreach (TaskEventHandler tpc in tpcs)
            {
                if (tpc.Target is System.Windows.Forms.Control)
                {
                    ...
                    targetForm.Invoke(tpc, new object[] { this, args });
                }
                else
                {
                    //tpc.BeginInvoke(this, args ,null,null); //异步调用,启动后不管
                    tpc.Invoke(this, args);
                }
            }
        }
        #endregion
        ...
        public bool StartTask(TaskDelegate worker, params object[] args)
        {
            ...
            lock (this)
            {
                if (IsStop && worker != null)
                {
                    ...
                    // 触发任务工作状态变化事件
                    ActivateStatusChangedEvent(_taskState, null);
                    result = true;
                }
            }
            return result;
        ...
        public bool StopTask()
        {
            ...
            lock (this)
            {
                if (_taskState == TaskStatus.Running)
                {
                    ...
                    // 触发任务工作状态变化事件
                    ActivateStatusChangedEvent(_taskState, _result);
                    result = true;
                }
            }
            return result;
        ...
        protected void EndWorkBack(IAsyncResult ar)
        {
            ...
            catch (Exception e)								//如果错误,则保存错误对象
            {
                ...
                {
                    abort = true;
                    ActivateAbortEvent(_progress, _exception);
                }
                else
                {
                    ActivateThrowErrorEvent(_progress, _exception);
                }
            }
            lock (this)
            {
