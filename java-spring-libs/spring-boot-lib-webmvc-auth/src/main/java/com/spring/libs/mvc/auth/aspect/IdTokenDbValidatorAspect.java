...
@Aspect
@Component
public class IdTokenDbValidatorAspect {

    @Autowired(required = false)
    IdTokenDbValidateService idTokenDbValidateService;

    @Pointcut("@annotation(com.ef.edtech.libs.mvc.auth.annotation.IdTokenDbValidator)")
    ...
    public void doBeforeAdvice(JoinPoint joinPoint) throws Exception {
        ...
        if (Objects.nonNull(idTokenDbValidateService)) {
            idTokenDbValidateService.validate((JwtContext) optional.get());
        }
    }
}
